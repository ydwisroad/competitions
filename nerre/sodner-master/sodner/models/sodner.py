...
class SodNER(Model):
    ...
    #dict_keys(['text', 'spans', 'ner_labels', 'relation_labels', 'metadata', 'dep_span_children'])
    @overrides
    def forward(self,
                text,    #text['bert'] => [8,21] , text['bert-type-ids']=> [8,21] , text['bert-offsets'] => [8,18], text['mask'] => [8,18]
                spans,   #[8, 93, 2]
                ner_labels, #[8, 93]
                relation_labels, #[8, 93, 93]
                metadata, #
                dep_span_children,  #[8, 18, 18]
                ):

        # In AllenNLP, AdjacencyFields are passed in as floats. This fixes it.
        relation_labels = relation_labels.long()

        text_embeddings = self._text_field_embedder(text)  # =>[8, 18, 768]

        text_embeddings = self._lexical_dropout(text_embeddings) # =>[8, 18, 768]

        # Shape: (batch_size, max_sentence_length)
        text_mask = util.get_text_field_mask(text).float()  # => [8,18]

        sentence_lengths = 0*text_mask.sum(dim=1).long()   # [8]
        for i in range(len(metadata)):
            sentence_lengths[i] = metadata[i]["end_ix"] - metadata[i]["start_ix"]

        # Shape: (batch_size, max_sentence_length, encoding_dim)
        contextualized_embeddings = self._lstm_dropout(self._context_layer(text_embeddings, text_mask))  #[8, 18, 400]
        assert spans.max() < contextualized_embeddings.shape[1]

        if self.use_dep:
            dep_span_children = dep_span_children + 1
            dep_feature_embeddings = self._dep_tree(dep_span_children, contextualized_embeddings, text_mask)  #[8, 18, 20]
            contextualized_embeddings = torch.cat([contextualized_embeddings, dep_feature_embeddings], dim=-1)  #[8, 18, 420]

        # Shape: (batch_size, num_spans)
        span_mask = (spans[:, :, 0] >= 0).float() #[8,93]
        spans = F.relu(spans.float()).long()  #[8, 93, 2]

        # Shape: (batch_size, num_spans, 2 * encoding_dim + feature_size)
        span_embeddings = self._endpoint_span_extractor(contextualized_embeddings, spans, text_mask) #[8, 93, 860]
